...
class CentroidWordEmbeddingsSummarizer_ORG(base.BaseSummarizer):
    def __init__(self,
                 embedding_model,
                 language='english',
    ...
    def compose_vectors(self, words):
        composed_vector = np.zeros(50, dtype="float32") # self.embedding_model.vector_size
    ...
    def summarize(self, text, limit_type='word', limit=100):
        ...
        for i in range(len(clean_sentences)):
            ...
            #xu ly cau thanh object sentence
            #

            if self.debug:
                print(i, scores, score)
        ############################
        sentence_scores_sort = sorted(sentences_scores, key=lambda el: el[2], reverse=True)[:10]

        list_sents = [s[1] for s in sentence_scores_sort]

        sentences_summary = mmr_summarizer.getMMR(list_sents)
