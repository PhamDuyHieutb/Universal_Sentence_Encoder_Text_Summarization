...
def concate_features(cnn_features, svm_features_path, option):
    ...
    list_files = []
    if option == 'train':
        list_files = open('/home/hieupd/PycharmProjects/CNN_SVM_summarization/Models/CNN_eng/train_files.txt', 'r').read().split('\n')
    else:
        list_files = open('/home/hieupd/PycharmProjects/CNN_SVM_summarization/Models/CNN_eng/test_files.txt', 'r').read().split('\n')
...
def get_position_features(vectors):
    length = len(vectors)
    arr_pos_feas = []
    for i in range(length):
        if i < length - 1:
            arr_pos_feas.append([1/(i + 1)])
        else:
            arr_pos_feas.append([1])

    return np.concatenate((np.array(arr_pos_feas), vectors), axis=1)